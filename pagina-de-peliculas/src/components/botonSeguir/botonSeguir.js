...
const BotonSeguir = ({ usuario, actualizarDatos }) => {
    ...
    const seguir = async (id_usuario_seguidor, id_usuario_seguido) => {
        ...
        const response = await fetch("https://13.48.181.115/seguidor", {
            method: "POST",
            headers: {
        ...
        const responseSeguidores = await fetch(`https://13.48.181.115/seguidores/${usuario.id}`);
        const dataSeguidores = await responseSeguidores.json();
        setSeguidores(dataSeguidores);

        const responseSeguidos = await fetch(`https://13.48.181.115/seguidos/${usuario.id}`);
    ...
    const dejarDeSeguir = async (id_usuario_seguidor, id_usuario_seguido) => {
        const dejarSeguir = await fetch(`https://13.48.181.115/seguidor/${id_usuario_seguidor}/${id_usuario_seguido}`, {
            method: "DELETE"
        });
    ...
    const editarPerfil = async () => {
        if (envioEditar) {
            if (error === false) {
                ...
                const response = await fetch(`https://13.48.181.115/usuario/${userData.id}`, {
                    method: "PUT",
                    headers: {
                        "Content-Type": "application/json",
                    },
